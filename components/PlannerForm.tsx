...
const PlannerForm: React.FC<PlannerFormProps> = ({ onPlanTrip, error }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!destination || duration < 1 || interests.length === 0) {
      alert("Please fill in all fields and select at least one interest.");
      return;
    }
    onPlanTrip({ destination, duration, interests, budget });
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden p-8 sm:p-12 space-y-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <div className="text-center">
             <label htmlFor="destination" className="block text-xl font-medium text-gray-500 mb-4">Where are you dreaming of going?</label>
             <input
                type="text"
                id="destination"
                value={destination}
                onChange={e => setDestination(e.target.value)}
                className="w-full sm:w-3/4 mx-auto bg-transparent border-b-2 border-gray-300 focus:border-brand-primary focus:outline-none text-4xl sm:text-5xl font-bold text-center text-brand-dark font-serif tracking-tight py-3 transition-colors"
          ...
          <div className="text-center pt-6">
            ...
            <button
              type="submit"
              className="w-full sm:w-auto inline-flex items-center justify-center px-12 py-4 border border-transparent text-lg font-semibold rounded-full shadow-lg text-white bg-brand-primary hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary transform hover:scale-105 transition-transform"
            >
              Plan My Trip
              <PaperAirplaneIcon className="h-5 w-5 ml-3" />
            </button>
          </div>
        </form>
      </div>
